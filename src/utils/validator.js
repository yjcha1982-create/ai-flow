  ...
  flows.forEach((flow) => {
    ...
    flow.nodes.forEach((node) => {
      ...
      if (data.childFlowId && !project.flows[data.childFlowId]) {
        issues.push(issue('error', 'CHILD_FLOW_MISSING', '연결된 하위 Flow를 찾을 수 없습니다.', flow.id, node.id));
      }
    });
    ...
    flow.edges.forEach((edge) => {
      ...
      if (
        rules.requireConditionPrompt &&
        data.relationType === 'condition' &&
        !data.prompt?.trim() &&
        !data.condition?.trim()
      ) {
        issues.push(issue('error', 'CONDITION_REQUIRED', '조건 관계에는 조건 또는 프롬프트가 필요합니다.', flow.id, edge.id));
      }
    });
  });
  ...
  flows.forEach((flow) => {
    if (flow.parentBoxId) {
      ...
      if (!owner || owner.box.data.childFlowId !== flow.id) {
        issues.push(issue('error', 'FLOW_OWNER_INVALID', '하위 Flow와 소유 Box 연결이 올바르지 않습니다.', flow.id));
      }
    }
  });
